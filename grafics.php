...
function allNoteAlumns(){
   ...
   // $resultat = $mysqli -> query("SELECT NOTA from NOTA" );
    while($fila=$resultat->fetch_assoc()){
        $array_notes[$fila["COGNOM1_ALUMNE"]] = $fila["NOTA"];
    }
   declaraGlobals($array_notes);
...
function cursNoteAlumns($curs){
    ...
    {
        $array_notes[$cognom1_alumne] = $nota;
    }
    declaraGlobals($array_notes);
}
...
function cursNoteAlumnsAssignature($assign, $curs){
    ...
    function dibuixaBarres($draw, $img) {
        ...
        $textPosicioVertical = $GLOBALS['vertical'] - $pixels/45;
     ...
     *
     * font: tipus de lletra per al text del gràfic
     *
     ...
     * radi: radi del diagrama de sectors, és proporcional al tamany de la imatge
     *
    ...
    function declaraGlobals($array_notes) {
        ...
        $GLOBALS['pixels'] = 45;
        $GLOBALS['marge'] = 20;
        $GLOBALS['horitzontal'] = $GLOBALS['alumnes']*$GLOBALS['pixels'] + 2*$GLOBALS['marge'];
        $GLOBALS['vertical'] = 500 + $GLOBALS['pixels'];
